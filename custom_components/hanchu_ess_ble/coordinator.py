    ...
    def _build_data( 
        ...
        *, 
        ...
        if values is not None: 
            merged_values = ( 
                dict(self.data.values) 
                if self.data is not None and self.data.values 
                else {} 
            ) 
            merged_values.update(values) 
        else: 
            merged_values = self.data.values if self.data is not None else None 
        ...
        if values is not None: 
            merged_ts: dict[str, datetime] = ( 
                dict(self.data.last_updated) 
                if self.data is not None and self.data.last_updated 
                else {} 
            ) 
            for key in values: 
                merged_ts[key] = now 
        else: 
            merged_ts = self.data.last_updated if self.data is not None else None 
        ...
        return HanchuCoordinatorData( 
            ...
            last_successful_read=( 
                ...
                else (self.data.last_successful_read if self.data is not None else None) 
            ), 
            consecutive_failures=( 
                consecutive_failures 
                if consecutive_failures is not None 
                else (self.data.consecutive_failures if self.data is not None else 0) 
            ), 
            last_cycle_duration=( 
                last_cycle_duration 
                if last_cycle_duration is not None 
                else (self.data.last_cycle_duration if self.data is not None else None) 
            ), 
        ) 
