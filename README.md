...
- **Confirm Write is global per device, not per-edit** — if an automation triggers Confirm Write (or calls the underlying service) while you have an unrelated change staged on the dashboard but haven't pressed Confirm yourself, your staged change will be applied too, and the buttons will show as unavailable immediately afterward. This is expected given the buffer is shared per device rather than scoped per user or session. In practice this requires a manual edit and an automation-triggered confirm to overlap within the same short window, which is unlikely in typical single-user home use, but worth being aware of if you have automations that call Confirm Write directly.
