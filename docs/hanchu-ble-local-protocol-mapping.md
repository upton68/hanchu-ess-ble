...
Status as of September 2026. Compiled from: the `hanchu_ess_ble` fork's live BLE reads/writes (HA Green + M5Stack Atom Lite BLE proxy), the [1ulk Hanchu BLE Controller](https://github.com/1ulk/1ulk.github.io) (`hanchu-params.js` / `hanchu-controller.js`), and cross-referencing against the Hanchu cloud app and a working cloud-based HA integration.
...
- **Full BLE operation needs one outer timeout, not just connection setup** — `establish_connection()` isn't the only call that can hang under BLE proxy contention; `start_notify()`, `write_gatt_char()`, and especially `disconnect()` (called in a `finally` block) have no timeout of their own and can block indefinitely on some Bleak/BlueZ backends. Wrapping only the connection step is not enough — wrap the entire connect → notify → handshake → write → disconnect sequence in a single `asyncio.wait_for(..., timeout=60.0)` so a hang anywhere in the chain surfaces as a clean, catchable `TimeoutError` instead of freezing the coordinator's update cycle permanently with no exception and no failure logged.
- **`DataUpdateCoordinator` created inside another coordinator's `async_setup()` may not reliably reschedule itself** — `HanchuBatteryCoordinator` instances are created dynamically inside the parent inverter coordinator's `async_setup()`, rather than through HA's normal top-level `async_setup_entry()` flow. In testing, the base class's built-in periodic rescheduling ran the first refresh correctly but then never re-armed — entities went stale indefinitely with zero errors logged and failure counters frozen at 0, while a manual refresh (`homeassistant.update_entity`) always worked perfectly, confirming the read pipeline itself was healthy. Fixed by passing `update_interval=None` to disable the base class's own scheduling entirely, and instead driving refreshes explicitly via `homeassistant.helpers.event.async_track_time_interval` calling `coordinator.async_request_refresh()`. Don't do both (i.e. don't set a real `update_interval` *and* add an explicit timer) — the two schedulers can run concurrently, causing far more frequent, overlapping refresh attempts than intended and genuine race-condition errors from concurrent access to coordinator state.
- **A multi-entry write payload (multiple `{"k":...,"v":...}` objects in one `data` array) reports false success** — the read side is confirmed to accept a multi-entry `data` array (used for batched reads across `FAST_POLL_KEYS`/`SLOW_POLL_KEYS`), which suggested by analogy that a multi-key write might work the same way. Bench-testing this directly on real hardware (Sept 2026) found the opposite: the device replied with a `0` (success) status for every key in the batch, and an immediate read-back even echoed the new values — but the underlying registers were **not actually changed** on the device or reflected in the vendor app. Do not rely on a multi-entry write payload or its reply/read-back as confirmation that values were committed. The reliable pattern for applying several writes together is to open one BLE connection and send each as its own single-key `act:"3"` request in sequence before disconnecting (see `async_write_values()` in `ble_client.py`) — this avoids per-write connection overhead without depending on unverified multi-key write semantics.
- **A register can report a commanded value before the physical change has actually completed** — confirmed specifically for P500 (see below), where the register flips to the new value well before the inverter has finished a state transition that visibly takes much longer. Any consumer treating "register now reads the new value" as equivalent to "the physical change is finished" should verify that assumption per-register rather than assuming it generally, especially for registers tied to hardware transitions (relays, mode changes) rather than simple settings.
...
| **P500** | **Grid Relay / Inverter Power State** | **1 = On, 0 = Off — controls the grid relay and overall inverter power state; setting Off stops solar, battery charge/discharge, and EPS output together, equivalent to the physical AC isolator switch. Confirmed the register reports the commanded value well before the physical transition finishes: Off completes in ~7-10s and the register matches quickly, but On was observed reading back as "on" ~20s in during HA testing, despite the inverter still visibly restarting for another ~30s (~50s total, several relay clicks, per PaulDGAL's hardware testing). Treat "register matches" and "physically complete" as separate things for this register, not interchangeable.** | |
...
- **PaulDGAL** — real-world testing identifying BLE load and sign convention issues, proposing the tiered polling approach, discovering/mapping the RTC and firmware diagnostic registers (L020, L094, L096) later implemented in v1.0.10, and identifying the P500 register plus hands-on hardware testing of its full transition behaviour and timing (v1.3.0)
- **Dean (upton68)** — bench-testing that identified the multi-entry write false-success behaviour, and real-hardware testing that identified P500's early-confirmation timing discrepancy on power-on
